...
| `code_review` | `off` / `on` | `off` | Auditoria adversarial do código recém-escrito. `on`: quando a feature fecha, um subagente `code-reviewer` audita a feature inteira — bug, borda não tratada, contrato divergente da spec, erro engolido, vazamento, segurança. Uma passada por feature, não por chunk. Cada achado vem com ID curto, severidade, **cenário concreto de falha**, causa, correção sugerida e custo estimado; sem cenário, não entra. Com achado grave ou médio, o fluxo pergunta o que fazer — corrigir agora, adiar ou descartar — e a correção é aplicada pelo principal (até 2 achados) ou por um implementer. O que ficar sem decisão abre o próximo `/lp-continue`, antes do chunk começar. Cada achado guarda o desfecho no banco (`aberto`/`corrigido`/`descartado`/`adiado`), e o que fica aberto reaparece no fechamento da feature. O revisor reporta, nunca corrige. Acrescente critérios em `~/.sdd/code-review.md` (seus, todo projeto) e `.sdd/code-review.md` (do repo, versionado). |
...
| `auto_commit` | `full` / `suggest-only` / `off` | `suggest-only` | Git a cada chunk aprovado. `full`: commita de verdade (os arquivos do chunk + os artefatos de **decisão** do `.sdd/` — spec, plan, memória, contexto), exceto em branch protegida (main/master/develop/staging/...). `suggest-only`: mostra o comando pronto pra copiar. `off`: não menciona git. Os artefatos de **progresso** (`tasks.md`, `flow.html`, `.sdd.yaml`) não entram no commit do chunk: acumulam e saem num `chore(sdd)` ao fechar a feature. |
