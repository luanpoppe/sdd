...
| `code_review` | `off` / `on` | `off` | Auditoria adversarial do código recém-escrito. `on`: um subagente `code-reviewer` revisa cada chunk e, de novo, a feature inteira ao fechar — bug, borda não tratada, contrato divergente da spec, erro engolido, vazamento, segurança. Cada achado vem com ID curto, severidade, **cenário concreto de falha**, causa, correção sugerida e custo estimado; sem cenário, não entra. Com achado grave ou médio, o fluxo pergunta o que fazer — corrigir agora, adiar ou descartar — e a correção é aplicada pelo principal (até 2 achados) ou por um implementer. Cada achado guarda o desfecho no banco (`aberto`/`corrigido`/`descartado`/`adiado`), e o que fica aberto reaparece no fechamento da feature. O revisor reporta, nunca corrige. Acrescente critérios em `~/.sdd/code-review.md` (seus, todo projeto) e `.sdd/code-review.md` (do repo, versionado). |
