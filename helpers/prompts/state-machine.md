...
> **Como citar chunks em qualquer texto explicativo.** O ID (`C6`, `F2.C3`) é **etiqueta de referência, não descrição** — ninguém lembra de cabeça o que era `C6`. Em toda prosa dirigida ao usuário (explicação do chunk, `Vem de`/`Prepara`, transições, resumo de onda, resposta a pergunta, plano de revisão), **nunca use o ID sozinho como se ele explicasse algo**. Sempre acompanhe do que aquele chunk faz: "o chunk que corrigiu a ausência de dimensão (`C6`)" — não "o `C6`". Se a frase continua clara sem o ID, prefira só a descrição; o ID entra entre parênteses quando o usuário pode querer referenciá-lo (revert, tasks.md). **Exceções** (ID cru é o certo): cabeçalhos de plano de revisão no formato `Chunk <ID> — <título>` (o título já vem ao lado), campos do `.sdd.yaml`/`tasks.md`, `data-*` do `flow.html`, e comandos que o usuário vai copiar ("reverte o chunk `F2.C3`").
