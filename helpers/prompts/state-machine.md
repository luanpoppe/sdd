...
| `implementing` | `lp:continue` | 1) Auto-sync: detectar e propor diff em divergências. 2) Pegar próximo chunk `[ ]` da `current_feature`. 3) Implementar — por **subagente** (default, `implementer: subagent`) ou pela conversa principal (`implementer: main`). 4) Marcar `[~]`. 5) Imprimir plano de revisão. | `implementing` (se há mais chunks) · `awaiting-feature-spec` (se feature done e há próxima) · `awaiting-archive` (se foi a última) |
