...
| `tests` (`off`) | `on`: no turno de fechamento da feature (ou da correção) — o `/lp-continue` seguinte ao último chunk — roda o passo **f-bis**: um tester dedicado escreve os testes da funcionalidade inteira, roda e **reporta sem corrigir**. Nunca por chunk. | `./tester-guide.md` |
...
| `flow_storage` (`file`) | Em `mcp`, o `flow.html` deixa de existir: o fluxo vira `component` + status no banco e quem desenha é o SDD Viewer (aba Fluxo). Exige `mcp: on`. | `./flowchart-guide.md` |
...
| `implementing` | `lp:continue` | 1) Auto-sync. 2) Definir modo (paralelo se `parallel: on` ou usuário pediu; senão sequencial). 3a) **Sequencial**: próximo chunk `[ ]`, explicação breve do chunk (o quê/por quê/conecta com macro/anteriores/próximos — timing conforme `implementer`), implementar por subagente (default) ou main. 3b) **Paralelo** (`./parallel-guide.md`): uma onda de chunks independentes, um subagente cada (sem a explicação breve — comunicação é por onda). 4) Marcar `[~]`. 4-bis) Se todos os chunks da feature ficaram prontos, **nada de fechamento agora**: anuncie que o próximo `/lp-continue` fecha a feature. 5) Plano de revisão (combinado no paralelo) + commit/sugestão de commit conforme `auto_commit`. 6) Com `mcp: on`, registrar o chunk e os arquivos no banco (g-bis). 7) **Turno de fechamento** (o `/lp-continue` seguinte, quando todos os chunks estão prontos e a feature ainda não é `done`): transição da feature (f), testes com `tests: on` (f-bis), code review com `code_review: on` (f-ter) e a decisão sobre os achados, que é uma porta — o que ficar `aberto` abre o próximo `/lp-continue` antes do chunk (ver `./code-review-guide.md`). O turno de fechamento não inicia chunk nem feature nova. | `implementing` (se há mais chunks/ondas) · `awaiting-feature-spec` (se feature done e há próxima) · `awaiting-archive` (se foi a última) |
...
1-bis. Se `tests: on`, roda o passo **f-bis**: subagente tester gera os testes da feature inteira, roda e reporta (ver `./tester-guide.md`). Com `tests: off`/ausente, pula em silêncio. Este passo e os demais desta transição acontecem no **turno de fechamento** — o `/lp-continue` seguinte ao que implementou o último chunk.
