...
- **A `spec.md` da feature** — em especial a seção **Requirements** e a seção **Edge cases**: são insumo direto de casos de teste, cada item deve virar pelo menos um teste. Os requisitos vêm em **dois formatos**, e ambos obrigam teste: BDD (`Dado que / Quando / Então`, com as afirmações seguintes em `E`) quando há ator externo, e `Entrada` / `Saída` / `Erro` quando o comportamento é interno. O formato técnico já traz o valor literal de entrada e saída — use exatamente esse valor, não invente outro. A linha `Erro` de um requisito é caso de teste como qualquer outro.
