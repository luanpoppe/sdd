...
4-bis. **Testes** (se `tests: on`): se esta onda **fechou a feature**, não rode nada agora — anuncie que o próximo `/lp-continue` fecha a feature. O passo f-bis roda lá, no turno de fechamento, igual ao sequencial: um subagente tester para a feature inteira, uma vez só (não um por chunk da onda). Ver `./tester-guide.md`.
