...
- **Carregue a memória**: leia a global `~/.sdd/memory.md` (lições sobre o fluxo `lp:*`; ausente é normal) e a do projeto `.sdd/memory.md` (ou `.sdd/memory-map.md` + arquivos relevantes pelo título da mudança). Siga `../../helpers/prompts/memory-guide.md`. Regra crítica: itens de **Estilo/Processo** aplicam direto; itens de **Stack/Domínio** só viram pergunta de confirmação ("Vi na memória que você usa X — confirma para esta mudança?"). Memória NUNCA substitui grill.
