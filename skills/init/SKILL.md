...
- on — cada etapa (chunk implementado, arquivos tocados e o que revisar em cada um, testes, divergências, steps de `lp:review-walkthrough`) também é gravada num SQLite em `~/.sdd/sdd.db`.
...
code_review: off        # off (padrão): nenhum review automático | on: um subagente code-reviewer audita cada chunk e a feature ao fechar, reportando achados com severidade e cenário de falha — nunca corrige nem bloqueia
data_model: off         # off (padrão): o implementer escreve a migração como qualquer outro arquivo | on: em chunk que toca dados, um subagente data-modeler desenha tabela/tipo/chave/índice ANTES, você aprova, e só então a migração é escrita. Ver helpers/prompts/data-model-guide.md.
...
flow_storage: file      # file (padrão): o diagrama vive em flow.html na pasta da mudança | mcp: o arquivo não existe e o fluxo é gravado no banco (rótulo do nó + status), desenhado pela aba Fluxo do SDD Viewer. Exige mcp: on. Ver helpers/prompts/flowchart-guide.md.
...
tasks_storage: file     # file (padrão): o plano de chunks vive no tasks.md, versionado no repo | mcp: o tasks.md não é gerado e o plano passa a viver no banco (nesse modo o MCP deixa de ser opcional). Ver helpers/prompts/mcp-guide.md.
state_storage: file     # file (padrão): o .sdd.yaml guarda tudo, inclusive state/current_chunk/in_review | mcp: só a identidade da mudança fica no arquivo e o bloco volátil passa a viver no banco (nesse modo o MCP deixa de ser opcional). Ver helpers/prompts/mcp-guide.md.
created: <YYYY-MM-DD>
```

> **`context`, `implementer`, `scribe`, `tasks_format`, `tasks_autocontinue`, `parallel`, `chunk_order`, `code_review`, `data_model` e `auto_commit` NÃO são perguntados no grill** — gravados com os defaults acima. `context: false` desliga a base de conhecimento. `implementer: main` faz a conversa principal implementar o código. `scribe: main` faz o principal escrever os artefatos inline. `tasks_format: follow` faz o `tasks` acompanhar o `format` global (gera `tasks.html`). `tasks_autocontinue: off` faz o `lp:continue` pausar após o `tasks.md` (em vez de seguir direto pro 1º chunk). `parallel: on` (ou `lp:parallel`) liga o modo paralelo; mesmo com `off`, o `lp:continue` pergunta uma vez antes do 1º chunk se você quer paralelizar. `chunk_order` muda a heurística de ordenação de features (lp:new-feature) e chunks (lp:continue) — ver `helpers/prompts/state-machine.md`. `tests: on` liga a geração automática de testes ao final de cada feature/correção (desligado por padrão) — ver `helpers/prompts/tester-guide.md`. `data_model: on` faz um subagente desenhar o modelo e esperar sua aprovação antes de a migração ser escrita, em chunk que toca dados — ver `helpers/prompts/data-model-guide.md`. `auto_commit: full` faz o `lp:continue` commitar de verdade cada chunk aprovado (exceto em branch protegida); `off` desliga qualquer menção a git — ver `helpers/prompts/git-guide.md`. Quem quiser, edita esses campos no `.sdd/config.yaml` depois (ou via `lp:settings`). O `mcp` **é** perguntado (Q6), então grave o valor escolhido — ou `off`, se a pergunta foi pulada por causa da versão do Node. **`flow_storage`, `tasks_storage` e `state_storage` também não são perguntados**: grave `file` nos dois, a menos que venham do global. Os dois modos `mcp` tiram artefato do repo e tornam o servidor obrigatório — decisão que ninguém deve tomar respondendo a um grill de projeto novo.

> **Bloco `mcp_record`: não escreva.** Com `mcp: on`, tudo é registrado por padrão. O bloco opcional `mcp_record` existe só para DESLIGAR partes do registro (`symbols`, `diff`, `context`, `explain`, `scenarios`, `code_review`, `data_model`) quando alguma delas não compensar naquele projeto. O `diff` já é pulado sozinho com `auto_commit: full`, sem precisar do bloco. Ausente = tudo ligado, que é o desejado na maioria dos casos. Se vier do global, copie-o. Ver `../../helpers/prompts/mcp-rationale.md`.
...
- **Projeto novo/vazio**: crie só o `index.md` esqueleto (cabeçalho + seção "Áreas / funcionalidades" vazia). O contexto cresce conforme os fluxos (`lp:continue`/`lp:bug-fix`/`lp:review-walkthrough`) forem implementando/revisando.
- Cite no resumo final quantos arquivos de contexto foram semeados.

## 3-ter. Registro do MCP (só se `mcp: on`)

Grave o servidor no arquivo de MCP do harness em que você está rodando — `.mcp.json` na raiz do projeto (Claude Code) ou `.cursor/mcp.json` (Cursor). Siga `../../helpers/prompts/mcp-rationale.md`, seção "Registro no harness".
...
Próximo passo: /lp-new-feature <id-da-mudanca>
```
