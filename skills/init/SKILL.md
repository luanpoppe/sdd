...
> **Bloco `mcp_record`: não escreva.** Com `mcp: on`, tudo é registrado por padrão. O bloco opcional `mcp_record` existe só para DESLIGAR partes do registro (`symbols`, `diff`, `context`, `explain`, `scenarios`) quando alguma delas não compensar naquele projeto. Ausente = tudo ligado, que é o desejado na maioria dos casos. Se vier do global, copie-o. Ver `../../helpers/prompts/mcp-guide.md`.
