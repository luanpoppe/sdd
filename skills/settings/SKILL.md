...
| `mcp` | `off` / `on` | `off` | Histórico estruturado via MCP local. `on`: cada etapa (chunk, arquivos tocados e o que revisar em cada um, testes, divergências, steps de `lp:review`) também vai para o SQLite global `~/.sdd/sdd.db` — destrava a timeline no SDD Viewer e a memória entre conversas e projetos. Exige Node 23+ e reiniciar a sessão. Ver `../../helpers/prompts/mcp-guide.md` para o que é gravado e `../../helpers/prompts/mcp-rationale.md` para as consequências de ligar. |
...
3-quater. **`mcp` não é só uma linha no YAML — o servidor precisa estar registrado.** Ao ligar (`mcp: on`) no **projeto**:
   - Chece o Node (`node -v`). Abaixo de 23, **não ligue**: explique em uma linha que o banco usa `node:sqlite` e pare.
