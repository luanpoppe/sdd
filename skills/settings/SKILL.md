...
| `mcp_record` | bloco aninhado (`symbols`, `diff`, `context`, `explain`, `scenarios` → `true`/`false`) | (ausente = tudo `true`) | **Opcional.** Desliga partes do registro quando `mcp: on`, sem desligar o MCP inteiro. Ex: `symbols: false` para de gravar métodos com exemplos de entrada/saída. |
...
3-quinquies. **`mcp_record`, `tasks_storage` e `state_storage` têm consequência que precisa ser dita na confirmação.**
   - `mcp_record.<chave>: false` → diga o que para de ser gravado e que o já gravado permanece. Chaves válidas: `symbols`, `diff`, `context`, `explain`, `scenarios`. Ausência de uma chave = ligada.
