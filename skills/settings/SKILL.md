...
| `subagents` | bloco aninhado (papel → harness → `{model, effort}`) | (ausente) | **Opcional.** Em qual modelo/thinking cada papel de subagente roda: `implementer`, `scribe`, `explorer`, `tester`; harnesses `claude-code`, `cursor`, `codex`. Ausente = cada subagente herda o modelo do principal. Ver `../../helpers/prompts/subagents-guide.md`. |
...
| `mcp_record` | bloco aninhado (`symbols`, `diff`, `context`, `explain`, `scenarios`, `code_review` → `true`/`false`) | (ausente = tudo `true`) | **Opcional.** Desliga partes do registro quando `mcp: on`, sem desligar o MCP inteiro. Ex: `symbols: false` para de gravar métodos com exemplos de entrada/saída. `diff` já é pulado sozinho quando `auto_commit: full`, porque o git guarda o mesmo diff. |
