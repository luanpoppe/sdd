...
| `code_review` | `off` / `on` | `off` | Auditoria adversarial do código recém-escrito. `on`: um subagente `code-reviewer` revisa cada chunk (e mais uma vez ao concluir a feature) procurando bug, borda não tratada, contrato divergente da spec, erro engolido, vazamento e falha de segurança. **Reporta com severidade e cenário concreto de falha, nunca corrige, nunca bloqueia.** Instruções extras em `~/.sdd/code-review.md` e `.sdd/code-review.md`. Ver `../../helpers/prompts/code-review-guide.md`. |
| `data_model` | `off` / `on` | `off` | Modelagem de dados antes da migração. `on`: em chunk que toca dados, um subagente `data-modeler` desenha tabela/tipo/chave/constraint/índice e a ordem da migração, **você aprova**, e só então a migração é escrita (o implementer fica com o código que a usa). **Único passo que bloqueia** — migração aplicada não volta com `git checkout`. Ver `../../helpers/prompts/data-model-guide.md`. |
| `subagents` | bloco aninhado (papel → harness → `{model, effort}`) | (ausente) | **Opcional.** Em qual modelo/thinking cada papel de subagente roda: `implementer`, `scribe`, `explorer`, `tester`, `code-reviewer`, `data-modeler`; harnesses `claude-code`, `cursor`, `codex`. Ausente = cada subagente herda o modelo do principal. Ver `../../helpers/prompts/subagents-guide.md`. |
| `mcp` | `off` / `on` | `off` | Histórico estruturado via MCP local. `on`: cada etapa (chunk, arquivos tocados e o que revisar em cada um, testes, divergências, steps de `lp:review-walkthrough`) também vai para o SQLite global `~/.sdd/sdd.db` — destrava a timeline no SDD Viewer e a memória entre conversas e projetos. Exige Node 18+ e reiniciar a sessão. Ver `../../helpers/prompts/mcp-guide.md` para o que é gravado e `../../helpers/prompts/mcp-rationale.md` para as consequências de ligar. |
| `flow_storage` | `file` / `mcp` | `file` | Onde vive o diagrama de fluxo. `file`: `flow.html` na mudança, o maior artefato do SDD (7k-32k tokens em projeto real). `mcp`: o arquivo **não existe** — o fluxo vira `component` + status no banco e quem desenha é a aba Fluxo do SDD Viewer. **Exige `mcp: on`.** Não migra nem apaga `flow.html` que já existe. Ver `../../helpers/prompts/flowchart-guide.md`. |
| `tasks_storage` | `file` / `mcp` | `file` | Onde vive o plano de chunks. `file`: `tasks.md` no repo, versionado e revisável em PR. `mcp`: o `tasks.md` não é gerado e o plano fica no banco — **nesse modo o MCP deixa de ser opcional** e o plano some do repo. Ver `../../helpers/prompts/mcp-guide.md`. |
| `state_storage` | `file` / `mcp` | `file` | Onde vive a parte do `.sdd.yaml` que muda a cada passo (`state`, `current_feature`, `current_chunk`, `in_review`, `updated` e o `status` de cada feature). `file`: tudo no arquivo, como sempre. `mcp`: esses campos saem do arquivo e ficam no banco — a identidade da mudança (`id`, `title`, `created`, `format`, `lang`, `chunk_size` e a lista de features) continua versionada. **Nesse modo o MCP deixa de ser opcional.** Ver `../../helpers/prompts/mcp-guide.md`. |
| `mcp_record` | bloco aninhado (`symbols`, `diff`, `context`, `explain`, `scenarios`, `code_review`, `data_model` → `true`/`false`) | (ausente = tudo `true`) | **Opcional.** Desliga partes do registro quando `mcp: on`, sem desligar o MCP inteiro. Ex: `symbols: false` para de gravar métodos com exemplos de entrada/saída. `diff` já é pulado sozinho quando `auto_commit: full`, porque o git guarda o mesmo diff. |
| `auto_commit` | `full` / `suggest-only` / `off` | `suggest-only` | Git a cada chunk aprovado. `full`: commita de verdade (git add + commit dos arquivos do chunk **mais os artefatos de decisão do `.sdd/`** — spec, plan, memória, contexto), exceto em branch protegida (main/master/develop/staging/... — aí só sugere, a menos que peça explicitamente). `suggest-only`: só mostra o comando pronto pra copiar. `off`: não menciona git. Os artefatos de progresso (`tasks.md`, `flow.html`, `.sdd.yaml`) saem num `chore(sdd)` ao fechar a feature — ver `../../helpers/prompts/git-guide.md`. |
...
Configurações do SDD (.sdd/config.yaml):
  ...
  tests=<v>               (off | on)
  mcp=<v>                 (off | on)
  tasks_storage=<v>       (file | mcp)
  state_storage=<v>       (file | mcp)
  mcp_record=<resumo>     (opcional — o que o MCP grava; "tudo ligado" se ausente)
...
3-quater. **`mcp` não é só uma linha no YAML — o servidor precisa estar registrado.** Ao ligar (`mcp: on`) no **projeto**:
   - Chece o Node (`node -v`). Abaixo de 18, **não ligue**: explique em uma linha qual é o piso e pare.
   - Grave o registro no `.mcp.json` (ou `.cursor/mcp.json`) fazendo **merge**, como descrito no passo 3-ter do `lp:init` e em `../../helpers/prompts/mcp-guide.md`. Sem isso o campo fica ligado e nada acontece — é a confusão mais previsível dessa config.
   - Feche dizendo que as tools só aparecem **após reiniciar a sessão**.
   - Ao desligar (`mcp: off`), deixe o `.mcp.json` como está e diga isso: o registro é inofensivo desligado, e removê-lo mexeria num arquivo versionado do time. Se o usuário pedir para remover, remova só a chave `sdd`.
   - No **global**, só grave o campo. Registrar `.mcp.json` é por projeto — quem faz isso é o `lp:init` do próximo projeto.
3-quinquies. **`mcp_record`, `tasks_storage` e `state_storage` têm consequência que precisa ser dita na confirmação.** A lista completa está em `../../helpers/prompts/mcp-rationale.md`, seção "Consequências de ligar cada modo" — leia-a antes de aplicar qualquer um dos três.
   - `mcp_record.explain: false` → caso especial que vale dizer: o `lp:explain` continua escrevendo o HTML em `~/.sdd/explain/` e a fila de estudo continua funcionando (o estado mora no `data-status` do arquivo). O que se perde é a busca alcançar os temas e a fila aparecer no SDD Viewer.
- `mcp_record.<chave>: false` → diga o que para de ser gravado e que o já gravado permanece. Chaves válidas: `symbols`, `diff`, `context`, `explain`, `scenarios`, `code_review`, `data_model`. Ausência de uma chave = ligada, **com uma exceção**: `diff` não é gravado quando `auto_commit: full`, mesmo ausente. Se o usuário perguntar por que o diff não aparece nesse modo, é essa a resposta — e `mcp_record.diff: true` não força o contrário.
   - `tasks_storage: mcp` → avise, na mesma resposta, que o `tasks.md` deixa de ser gerado, que o plano sai do repo (some da revisão de PR) e que **com esse modo o MCP vira obrigatório**: se o servidor não estiver disponível, o fluxo trava em vez de degradar. Só aplique depois disso dito.
   - `tasks_storage: file` de volta → o `tasks.md` volta a ser gerado a partir do próximo passo, mas o plano que ficou no banco **não** é convertido em arquivo automaticamente; diga isso.
   - `state_storage: mcp` → avise, na mesma resposta, que `state`, `current_feature`, `current_chunk`, `in_review`, `updated` e o `status` das features deixam de existir no `.sdd.yaml`, que o `git log` do arquivo para de mostrar o avanço da mudança e que **com esse modo o MCP vira obrigatório**: sem o servidor, o agente não sabe em que passo a mudança está e trava em vez de degradar. Só aplique depois disso dito.
   - `state_storage: file` de volta → na próxima escrita do `.sdd.yaml`, reescreva o bloco volátil a partir do `sdd_read_state`, para o arquivo não voltar vazio. Diga que fez isso.
3-bis. **`subagents` é aninhado — trate diferente dos campos de linha única.** É o único campo em bloco:
   - Não valide o nome do modelo contra lista fixa (o catálogo de cada harness muda) — grave o que o usuário pediu. Valide só o **papel** (`implementer`/`scribe`/`explorer`/`tester`) e, se o usuário nomear um, o **harness** (`claude-code`/`cursor`/`codex`).
...
- **Escopo do projeto**: a config do projeto vale pro projeto todo; muda o comportamento das próximas execuções (`lp:new-feature`, `lp:continue`, `lp:bug-fix`). Não reprocessa o que já foi gerado.
