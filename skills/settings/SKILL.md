...
| `mcp` | `off` / `on` | `off` | Histórico estruturado via MCP local. `on`: cada etapa (chunk, arquivos tocados e o que revisar em cada um, testes, divergências, steps de `lp:review`) também vai para o SQLite global `~/.sdd/sdd.db` — destrava a timeline no SDD Viewer e a memória entre conversas e projetos. Exige Node 18+ e reiniciar a sessão. Ver `../../helpers/prompts/mcp-guide.md` para o que é gravado e `../../helpers/prompts/mcp-rationale.md` para as consequências de ligar. |
...
| `mcp_record` | bloco aninhado (`symbols`, `diff`, `context`, `explain`, `scenarios` → `true`/`false`) | (ausente = tudo `true`) | **Opcional.** Desliga partes do registro quando `mcp: on`, sem desligar o MCP inteiro. Ex: `symbols: false` para de gravar métodos com exemplos de entrada/saída. `diff` já é pulado sozinho quando `auto_commit: full`, porque o git guarda o mesmo diff. |
